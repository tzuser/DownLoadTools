...
const main=async()=>{
  let cursor;
  cursor=postModel.find().populate('user').sort({_id:-1}).cursor()
  let doc;
  while(doc=await cursor.next()){
    let {name:userID,avatar}=doc.user;

    await download({src:getImgURL(avatar),userID,serverPath}).catch(err=>{
        console.log(`avatar download error ${err}`)
    })
    
    if(doc.thumbnail && doc.thumbnail.url){
      //下载缩略图
      let src=await download({src:getImgURL(doc.thumbnail.url),userID,serverPath}).catch(err=>{
    ...
    for(var photo of doc.photos){
      ...
      let src=download({src:getImgURL(photo.url),userID,serverPath}).catch(err=>{
        throw `photo download error ${err}`
      })
